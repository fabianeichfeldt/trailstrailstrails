 ...
 * Results go to <cache>/results.json next to the printed report.
 *
...
const cacheDir = args.get('cache') ?? join(tmpdir(), 'trailradar-soil-backtest')
const outFile = args.get('out') ?? join(cacheDir, 'results.json')
...
writeFileSync(
  outFile,
  JSON.stringify({ generatedAt: new Date().toISOString(), layer, rows, correlations, failures }, null, 2),
)
