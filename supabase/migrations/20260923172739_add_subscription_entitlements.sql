...
-- See docs/superpowers/specs/2026-09-23-subscription-entitlements-design.md.
...
-- Seed the three tiers so the backfill below (section 4) has a 'pro' row to
-- reference. Prices are NULL until real pricing is decided — a business
-- decision outside this migration's scope, not a blocker for the schema.
...
-- =============================================================================
-- Backfills every user that exists at the moment this migration is applied.
-- No trigger: signups after this point never get a row.
--
-- SEED DEFAULTS — tune before this migration is applied to a real project:
-- 'pro' (top tier at launch), 6 months free, 20% discount. These are values,
